---
title: Example application of the Cologne Protocol for the Early Neolithic of central Europe using R
...
## Task 1: loading data (Shape-layer with sites as points)
...
The "Largest Empty Circle" (LEC) or more precisely the radius/diameter of a LEC is a measure of site distance. Every LEC has it's center at a vertex of a Thiessen polygon (e.g. Voronoi diagram) and exactly three sites are located on every periphery of a LEC. An illustration can be found at *Zimmermann et al. (2004, Abb.5)*.  
...
### Tasks 2 and 3: Creating of Voronoi diagrams and Extraction of vertices
...
### Task 5: Defining the radius of the "Largest Empty Circle"
...
With the help of the experimental semivariogram, which you can seen below, we can compute a theoretical semivariogram.
...
Now we have every information for the kriging procedure. The only three variables we have to define are `nmin`, `nmax` and `maxdist` in the `gstat::krige()` function. `nmin` and `nmax` are in our case the minimum and maximum number of Voronoi vertices, which will be considered by the kriging algorithm, so we are using local Kriging. Furthermore, we define that only observations (vertices) within a range of half of the bounding box diagonal will be used (`maxdist`). These are the default values for the Cologne Protocol.
...
### Task 8: Kriging -  inspect and export raster output
...
### Task 9: Creating Contour Lines (Isolines)
...
In our sample application isolines will be created starting at 0,5 km and end up at 20 km with an equidistance of 0,5 km.
...
### Task 10: Calculating the Area and Number of Sites per Isoline
...
The next three calculations consider the number of sites within a LEC redius (certain site distance) and its percentage as well as the enclosed area:  
